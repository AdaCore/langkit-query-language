...
class InterpreterDriver(BasicTestDriver):
    ...
    interpreter's output ro the provided output file.

    The LKQL script to run must be placed in a file called script.
    The expected output must be written in a file called output.

    Test arguments:
        - project: relative path of the GPR build file to use (if any), from ADA_PROJECTS_PATH
        - failure (optional): True if the interpreter's execution must end with a non-zero exit code,
                   False otherwise
    """
    ...
    def run(self, prev):
        ...
        # if there is a difference between the process's output and the expected output,
        # or between the process's exit status and the expected exit status, the test fails
