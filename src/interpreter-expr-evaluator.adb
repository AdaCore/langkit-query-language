package body Interpreter.Expr.Evaluator is
   ...
   function Eval_Number (Ctx: in out EvalCtx; Node: in LEL.Number) return Atom is
   begin
      return (Kind => Number, Number_Val => Float'Wide_Wide_Value (Node.Text));
   end Eval_Number;
   
   
   function Eval_String_Literal (Ctx: in out EvalCtx; Node: in LEL.String_Literal) return Atom is
   ...
   function Eval_Bin_Op (Ctx: in out EvalCtx; Node: in LEL.Bin_Op) return Atom is
      Left: Atom := Eval (Ctx, Node.F_Left);
      Right: Atom := Eval (Ctx, Node.F_Right); 
   begin
      case Node.F_Op is
