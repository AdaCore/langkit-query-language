...
package body Gnatcheck.Rules.Rule_Table is
   ...
   --  Check that provided ``Rule`` is designating an existing rule, return
   ...
   --  Check that the given instance name isn't already registered in the
   --  global instance table. If not return ``True``, otherwise display an
   --  error message about this instance already existing and cannot being
   --  instantiated at ``Instantiation_Location``, set ``Bad_Rule_Detected``
   --  to ``True``, then return ``False``.
