...
package body Gnatcheck.Rules is
   ...
   procedure Print_Args_To_Universal_File
     (Args : Rule_Argument_Vectors.Vector;
      Rule_File : File_Type);
   --  Display the given arguments to the given rule file. This display
   --  follows the universal rule file format.
   ...
   -----------------------------------
   -- Print_Args_To_Universal_File --
   -----------------------------------

   procedure Print_Args_To_Universal_File
     (Args : Rule_Argument_Vectors.Vector;
      Rule_File : File_Type) is
   begin
   ...
   end Print_Args_To_Universal_File;
   ...
   begin
      ...
      --  the parameter value and enable the instance.
         ...
         --  Special case: "headers" rule takes a file name as parameter,
         ...
         if Rule_Name (Instance) = "headers" then
            declare
               ...
               Last : Natural;
            begin
               --  Try reading the provided file
               if Name /= "" then
                  Str := Read_File (Name);
                  Append (Tagged_Instance.File, Name);
               else
               ...
               if Last > 0 then
                  ...
                  Append
                    (Tagged_Instance.Param,
                     To_Wide_Wide_String (Str (1 .. Last)));
   ...
   begin
      ...
      --  instance process the parameter.
         ...
         --  should be read by the `Load_Dictionnary` function.
   ...
   -------------------------------------------
   -- Print_Rule_Instance_To_Universal_File --
   -------------------------------------------
   ...
   is
      Args : Rule_Argument_Vectors.Vector;
   begin
      --  Fetch instance argument to the argument vector
      Map_Parameters (Instance, Args);

      --  Print instance and its argument to the rule file
      if Instance.Is_Alias then
         Put (Rule_File, Instance_Name (Instance) & "|" &
                         Rule_Name (Instance));
      else
         Put (Rule_File, Rule_Name (Instance));
      end if;
      Print_Args_To_Universal_File (Args, Rule_File);
   end Print_Rule_Instance_To_Universal_File;
