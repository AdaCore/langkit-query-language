...
package body Gnatcheck.Rules is
   ...
   procedure Print_Args_To_Universal_Files
   ...
   end Print_Args_To_Universal_Files;
   ...
   begin
      ...
      Print_Args_To_Universal_Files (Args, Rule_File);
   end Print_Rule_To_Universal_File;

   -----------------------------------
   -- Print_Alias_To_Universal_File --
   -----------------------------------

   procedure Print_Alias_To_Universal_File
     (Alias     : in out Alias_Template'Class;
      Rule_File : File_Type)
   is
      Args : Rule_Argument_Vectors.Vector;
      Rule : constant Rule_Template'Class := Get_Rule (Alias);
   begin
      Map_Parameters (Alias, Args);
      Put (Rule_File, To_String (Alias.Name) & "|" & Rule_Name (Rule));
      Print_Args_To_Universal_Files (Args, Rule_File);
   end Print_Alias_To_Universal_File;
      ...
      if Param = "" then
         ...
         if Rule.Name = "headers" then
            ...
            begin
               if Name /= "" then
                  Str := Read_File (Name);
                  Append (File_Value, Name);
               else
                  Error
                    ("(" & To_String (Rule.Name) &
                     "): cannot load file " & Param);
                  Bad_Rule_Detected := True;
                  State_Value       := Disabled;
                  return;
               end if;
               ...
               if Last > 0 then
                  ...
                  Append (Param_Value, To_Wide_Wide_String (Str (1 .. Last)));
