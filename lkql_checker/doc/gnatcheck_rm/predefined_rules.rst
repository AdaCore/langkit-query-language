...
options see ``Warning Message Control`` section of the GNAT User's Guide.

*gnatcheck* does not check itself if this or that construct would result
in issuing a warning, instead it compiles
an argument source with needed warning control compilation options,
analyses the warnings generated by the GNAT compiler and
includes the information about the warnings emitted into the
*gnatcheck* messages.

The rule should have a parameter, the format of the parameter should
be a valid ``static_string_expression`` argument of the GNAT pragma ``Warnings``
(see ``Pragma Warnings`` subsection in the ``GNAT Reference Manual``).

Note that in case of *gnatcheck* the
``s`` parameter that corresponds to the GNAT ``-gnatws`` option, disables
all the specific warnings, but not does not suppress the warning mode,
...
specified in ``Style Checking`` section of the ``GNAT User's Guide``
according to the rule parameter(s) specified.

*gnatcheck* does not check GNAT style rules itself, instead it compiles
an argument source with needed style check compilation options,
analyses the style warnings generated by the GNAT compiler and
