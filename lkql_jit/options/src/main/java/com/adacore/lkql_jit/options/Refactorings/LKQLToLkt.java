...
public class LKQLToLkt {
    ...
    public static Refactoring instantiate() {
        return (Refactoring.State state) -> {
            ...
            for (var fd : Refactoring.findAll(state.unit.getRoot(), n ->
                ...
                var namedFun = (Liblkqllang.BaseFunction) fd;
                ...
                for (var p : namedFun.fParameters().children()) {
                    ...
                    state.append(param.tokenEnd(), ": Any");
                }
    ...
    public static String lkqlToLkt(String lkqlFile) {
        var ctx = Liblkqllang.AnalysisContext.create();
        var unit = ctx.getUnitFromFile(lkqlFile);
