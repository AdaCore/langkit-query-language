...
/**
 * Auto-fix of a diagnostic, as computed by an LKQL auto-fix function.
 *
 * @param description Short description of what the fix does.
 * @param changes Changes to apply to the sources, one per modified source.
 */
public record AutoFix(String description, List<Change> changes) {
    /**
     * Change of a single source.
     *
     * @param targetSource Source to apply the patch to.
     * @param patch The patch computed from the auto-fix function.
     */
    public record Change(Source targetSource, Patch<String> patch) {
        // ----- Attributes -----
        ...
        /**
         * Get a pretty and easy to read representation of this change.
         *
        ...
        /** Create a new SARIF artifact change object representing this change and return it. */
            ...
            for (var batch : diffBatches) {
                ...
                // Otherwise, the batch is insertions or deletions
                    ...
                    if (batch.getFirst().diff.getTag() == DiffRow.Tag.INSERT) {
                        ...
                        // If the diff is a deletion, set the end of the deleted region to the start
                        // of the line next to the batch.
                        deletedRegion.setEndLine(batch.getLast().lineNum + 1);

                        // Then set the snippet of the deleted region to the text of the removed
                        // batch
                        var snippet = new ArtifactContent();
                        snippet.setText(
                            batch
        ...
        /**
         * Get all lines modified by this change associated to their 1-based number in the file.
         *
        ...
        private static List<List<DiffLine>> toBatches(
            ...
            for (var diffLine : diffLines) {
                if (
                    ...
                    (diffLine.lineNum > currentBatch.getLast().lineNum + 1 ||
                        (splitByTag &&
                            diffLine.diff.getTag() != currentBatch.getLast().diff.getTag()))
                ) {
