...
public final class ConstructorCall extends Expr {
    ...
    public final LangkitSupport.NodeKind nodeKind;
    ...
    public LangkitSupport.RewritingNode executeRewritingNode(VirtualFrame frame) {
        ...
        // Else, if the node is a list node, evaluate the first argument as a list
            ...
            if (this.args.length == 0) {
                return rewritingContext.createNode(
                    this.nodeKind,
                    new LangkitSupport.RewritingNode[0]
                );
            } else {
                try {
                    final var objectArgs = this.args[0].getArgExpr().executeList(frame).content;
                    final var args = new LangkitSupport.RewritingNode[objectArgs.length];
        ...
        // Else, evaluate all arguments as rewriting nodes and call the constructor
            ...
            final var args = new LangkitSupport.RewritingNode[this.args.length];
