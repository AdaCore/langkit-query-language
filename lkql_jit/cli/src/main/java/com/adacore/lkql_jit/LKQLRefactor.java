...
public class LKQLRefactor implements Callable<Integer> {
     ...
     * 'hashCode' method on Tokens is wrong. This should be fixed at the Langkit level.
     *
    ...
    private String getTokenId(Token token) {
        return token.unit.getFileName()
                + token.isTrivia()
                + (token.isTrivia() ? token.triviaIndex : token.tokenIndex)
                + token.kind;
    }
    ...
    public void addAction(Token token, Action action) {
        ...
        if (actions.containsKey(getTokenId(token))) {
            actionList = actions.get(getTokenId(token));
        } else {
            actionList = new ArrayList<>();
            actions.put(getTokenId(token), actionList);
        }
     ...
     * iterating on tokens from 'fromTok'.
     */
    ...
    /**
     * Internal method: rewrite an unit by printing all the tokens via the 'write' callback. Apply
