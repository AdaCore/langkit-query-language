...
public class LKQLDocRules implements Callable<Integer> {
    ...
    private static FunDecl isCheck(AnalysisUnit unit) {
        ...
        for (var fun : findAll(root, (n) -> n instanceof FunDecl)) {
            final DeclAnnotation ann = FunDecl.class.cast(fun).fAnnotation();
            if (ann != null && !ann.isNone() && ann.fName().pSym().text.endsWith("check"))
                return FunDecl.class.cast(fun);
        }
    ...
    private static class Rule implements Comparable<Rule> {
        ...
        public Rule(FunDecl check) {
            ...
            // casing.
            String name = this.getAnnotationArgument("rule_name");
            if (name == "") name = this.check.fName().pSym().text;
            this.name =
                    List.of(name.split("[_]")).stream()
                            .map(s -> s.substring(0, 1).toUpperCase() + s.substring(1))
                            .collect(Collectors.joining("_"));

            this.category = this.getAnnotationArgument("category");
            this.subcategory = this.getAnnotationArgument("subcategory");
        }

        /** Get the argument of annotation 'name' if it exists, empty string otherwise. */
        private String getAnnotationArgument(String name) {
            var ann = this.check.fAnnotation();

            if (ann != null && !ann.isNone() && ann.fName().pSym().text.endsWith("check")) {
                var arg = ann.pArgWithName(Symbol.create(name));
                if (!arg.isNone() && arg.pExpr() instanceof StringLiteral)
                    return arg.pExpr().getText().replaceAll("\"", "");
            }
            return "";
        }
        ...
        public String toRST() {
            ...
            if (doc instanceof StringLiteral) {
                ...
                for (var subBlocks : BlockStringLiteral.class.cast(doc).fDocs().children()) {
        ...
        /** Convert this rule to a String. Only for debug purposes. */
        @Override
    ...
    public Integer call() throws Exception {
        ...
        // Create rules objects, only keep check/unit_check FunDecls.
